    ...
    def __init__(self):
        with open("WGUPS Distance Table.csv") as csvfile:
            ...
            for i, row in enumerate(reader):
                ...
                if i == 0:
                    AddressList.address_row = edited_row[2:]
                else:
                    AddressList.address_col.append(edited_row[0])
                    AddressList.address_matrix.append(edited_row[2:])

    def calculate_distance(self, address1, address2):
        address1_col_index = -1
        address2_col_index = -1
        for i, entry in enumerate(AddressList.address_row):
        ...
        for i, entry in enumerate(AddressList.address_col):
            if address1 in entry:
        ...
        """
        if AddressList.address_matrix[address1_row_index][address2_col_index] == '':
            return float(AddressList.address_matrix[address2_row_index][address1_col_index])

        else:
            return float(AddressList.address_matrix[address1_row_index][address2_col_index])
